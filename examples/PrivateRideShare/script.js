// Private Ride Share DApp with Universal FHEVM SDK Integration
class PrivateRideShareApp {
    constructor() {
        this.provider = null;
        this.signer = null;
        this.contract = null;
        this.fhevmClient = null; // Universal SDK client
    ...
    async connectWallet() {
        try {
            ...
            // Initialize Universal FHEVM SDK
            try {
                // Import FHEVMClient from the Universal SDK
                ...
                if (FHEVMClient) {
                    ...
                    console.warn('Universal SDK not available, using fallback');
                    // Fallback to fhevmjs if SDK not available
                    this.fhevmClient = await createFhevmInstance({
            ...
            // Initialize contract (using SDK if available)
            if (this.fhevmClient && this.fhevmClient.getContract) {
                // Use SDK's contract wrapper
                this.contract = this.fhevmClient.getContract(this.contractAddress, this.contractABI);
            } else {
                // Fallback to regular ethers contract
    ...
    /**
     * Encrypt a value using Universal SDK if available
     * @param {number} value - Value to encrypt
     * @param {string} type - FHE data type (default: 'euint32')
     * @returns {Promise<any>} Encrypted value
     */
    async encryptValue(value, type = 'euint32') {
        if (this.fhevmClient && this.fhevmClient.encrypt) {
            // Use Universal SDK encryption
            try {
                const encrypted = await this.fhevmClient.encrypt(value, type);
                console.log(`Encrypted ${value} using Universal SDK`);
                return encrypted;
            } catch (error) {
                console.error('SDK encryption failed, using plain value:', error);
                return value;
            }
        }
        // Return plain value if SDK not available
        return value;
    }

    /**
     * Decrypt a value using Universal SDK if available
     * @param {any} handle - Encrypted handle
     * @param {string} type - FHE data type (default: 'euint32')
     * @returns {Promise<any>} Decrypted value
     */
    async decryptValue(handle, type = 'euint32') {
        if (this.fhevmClient && this.fhevmClient.decrypt) {
            // Use Universal SDK decryption
            try {
                const decrypted = await this.fhevmClient.decrypt(handle, type);
                console.log('Decrypted value using Universal SDK');
                return decrypted;
            } catch (error) {
                console.error('SDK decryption failed:', error);
                return null;
            }
        }
        return handle;
    ...
    async handleRequestRide(event) {
        ...
        const destLng = Math.floor(parseFloat(document.getElementById('destLng').value) * 1000000);
        const maxFare = parseInt(document.getElementById('maxFare').value);
